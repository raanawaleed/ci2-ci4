...
class UserModel extends Model
{
    ...
    public static function validateLogin($username, $password)
    {
        ...
        if ($user && $user->validatePassword($password) && $user->status === 'active') {
    ...
    {
        return $this->update($id, ['hashed_password' => $hashedPassword]);
    }

    public function getProjectsBySalaryId($salariesId)
    {
        return $this->db->table('projects')
            ->where('salaries_id', $salariesId)
            ->get()
            ->getResult();
    }
}
