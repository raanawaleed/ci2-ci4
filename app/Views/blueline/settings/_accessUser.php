...
<div class="form-group" id="<?php echo $value->name;?>" >
	...
	<select class="chosen-select inbox-folder" name="status" title="status" id="status">         
        <option value="active" <?= ($user->status == 'active' ) ? 'selected' : '' ?> class="primary-menu" >active</option>
        <option value="inactive" <?= ($user->status == 'inactive' ) ? 'selected' : '' ?> class="primary-menu" >inactive</option>
...
<div class="row">
    ...
    <div class="col-md-6">
				<div class="form-group" style="padding: 20px 9px;">
					...
					<label class="switch" style="text-align: center;" >
						<?php if(verifSalaries($user->salaries_id)=="true"){ ?>
...
<div class="form-group" id="<?php echo $value->name; ?>">
    ...
    <ul class="accesslist">
        <?php foreach ($modules as $module) {
            if ($module->default_module == 1) {
                $Module = ($module->link == "v_companies/revoke") ? $module->name : $module->link; ?>
                <li>
                    <input style="display:inline" type="checkbox" class="checkbox" id="<?= $module->id; ?>" name="defaultAccess[]" data-labelauty="<?= $this->lang->line('application_'.$Module); ?>" value="<?= $module->id; ?>" checked onclick="return false;">
                </li>
            <?php }
        } ?>
    </ul>
    ...
    <ul class="accesslist">
        <?php foreach ($modules as $module) {
            if ($module->default_module == 0) {
                $Module = is_null($module->link) ? $module->name : $module->link; ?>
                <li>
                    <input style="display:inline" type="checkbox" class="checkbox test" id="<?= $module->id; ?>" name="menu[]" data-labelauty="<?= $this->lang->line('application_'.$Module); ?>" value="<?= $module->id; ?>" onclick="ckeckedSubmodules(<?= $module->id; ?>);">
                    <?php // Sous menus
                    foreach ($submenus as $submenu) {
                        if ($submenu->id_modules == $module->id) {
                            if (in_array($submenu->id, $tabsubaccess)) { ?>
                                <ul>
                                    <li style="padding-left:20px">
                                        <input style="display:inline" type="checkbox" class="checkbox child" id="<?= $submenu->link; ?>" name="submenu[]" data-labelauty="<?= $this->lang->line('application_'.$submenu->name); ?>" value="<?= $submenu->id; ?>" onclick="ckeckedModule(<?= $submenu->link; ?>);" checked>
                                    </li>
                                </ul>
                            <?php } else { ?>
                                <ul>
                                    <li style="padding-left:20px">
                                        <input style="display:inline" type="checkbox" class="checkbox child" id="<?= $submenu->link; ?>" name="submenu[]" data-labelauty="<?= $this->lang->line('application_'.$submenu->name); ?>" value="<?= $submenu->id; ?>" onclick="ckeckedModule(<?= $submenu->link; ?>);">
                                    </li>
                                </ul>
                            <?php }
                        }
