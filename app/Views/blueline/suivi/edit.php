                ...
                <div class="calendars"> 
                    ...
                    <?php foreach($data['users'] as $user): ?>   
                            <div class="item" data-id="<?php echo($user->id) ?>" >
                                ...
                                <span class="name itemname" data-toggle="popover"
    ...
    $(".user").empty();
      $('.user').append('<?php foreach($data["usersmms"] as $user): ?>'+   
                            '<div class="item">'+
                               '<div class="name-column">'+
                                '<div class="name-wrap">'+
                                '<span class="name itemname" data-toggle="popover" data-placement="right" data-trigger="focus" data-html="true" data-title ="<?php echo($user->nom .' '.$user->prenom) ?>" data-ville ="<?php echo($user->ville) ?>"  data-affectation ="<?php echo($user->seraffectation) ?>"  data-file ="<?php echo($user->file); ?>" data-phone ="<?php echo($user->tel1) ?>"><?php echo get_salaries_icon($user->genre)?><?php echo($user->nom .' '.$user->prenom) ?></span>'+
                               '</div>'+
                                 '</div>'+
                                 '<div class="days">'+
                                    '<?php foreach ($data["data_m"] as $day): ?>'+
  ...
  if(select == 'BIM 2D')
   {
    ...
    $('.user').append('<?php foreach($data["users2d"] as $user): ?>'+   
                            '<div class="item">'+
                               '<div class="name-column">'+
                                '<div class="name-wrap">'+
                                '<span class="name itemname" data-toggle="popover"  data-file ="<?php echo($user->file); ?>" data-placement="right" data-trigger="focus" data-html="true" data-title ="<?php echo($user->nom .' '.$user->prenom) ?>" data-ville ="<?php echo($user->ville) ?>"  data-affectation ="<?php echo($user->seraffectation) ?>"  data-phone ="<?php echo($user->tel1) ?>"><?php echo get_salaries_icon($user->genre)?><?php echo($user->nom .' '.$user->prenom) ?></span>'+
                               '</div>'+
                                 '</div>'+
                                 '<div class="days">'+
                                    '<?php foreach ($data["data_m"] as $day): ?>'+
