...
<head>
  ...
  <script src="<?= base_url('assets/blueline/js/plugins/jquery-1.12.4.min.js') ?>?ver=<?= esc($core_settings->version); ?>"></script>
  ...
  <script src="<?= base_url('assets/blueline/js/accordion-menu.js') ?>"></script>
  <script src="<?= base_url('assets/blueline/js/message.js') ?>"></script>
  <script src="<?= base_url('assets/blueline/js/super-panel.js') ?>"></script>
  <!-- Google Font Loader -->
  <script type="text/javascript">
    WebFontConfig = {
      google: {
        families: ['Open+Sans:400italic,400,300,600,700:latin,latin-ext']
      }
    };
    (function() {
          ...
          <ul class="nav nav-sidebar" style="margin-left:0;margin-bottom:50px;">
            ...
            foreach ($submenuRight as $key => $val) {
              ...
              if ($val->id_modules == $module_id) {
                $existSubmenu = true; ?>
                <li id="<?= ($val->name); ?>" id="<?= strtolower($val->name); ?>" class="	<?php if ($act_uri == strtolower($val->link)) {
                                                                                          echo "active";
                                                                                        } ?>">
            ...
            <?php if ($existSubmenu == false) {
              ...
              foreach ($menu as $key => $value) {   ?>
                <li id="<?= ($value->name); ?>" id="<?= strtolower($value->name); ?>" class="<?php if ($act_uri == strtolower($value->name)) {
                                                                                            if ($act_uri == "invoices") {
                                                                                            } else {
                                                                                              echo "active";
                                                                                            }
                ...
                <?php foreach ($sticky as $value): ?>
                  <li>
                    <a href="<?= base_url() ?>projects/view/<?= $value->id; ?>">
                      <p class="truncate"><i class="ion-ios-clock <?php if (!empty($value->tracking)) {
                                                                    echo "fa-spin";
                                                                  } ?>"></i> <?= $value->name; ?> </p>
                      <div class="progress">
            ...
            if ($user_online && $val->link == "useronline") { ?>
              <ul class="nav nav-sidebar user-online menu-sub hidden-sm hidden-xs  enLigne">
                <ul id="menu-accordeon">
                  <li><a href="#">
                      <h4 style="margin-top:0;"><?= $this->lang->line('application_user_online'); ?></h4>
                    </a>
                    <ul>
