...
//////////$this->view_data['estimates']=$this->estimate_model->idsal($ids);
	...
	*/
	function facture($id){
		...
		$devis->notes ='&nbsp;Attachement (Réf: '. $devis->project_ref .')<div>ARIANA , LE '.$issue_date .'</div><div><br></div><div><div>La somme des heures passées sur ce dossier est '. $devis->calcul_heure .' Heures</div><div>La somme des quantités sur ce dossier est '. $devis->project_surface .' m²</div><div><br> Réf QUARTA: '. $refP.' 
		</div>'; 
		...
		foreach ($subject as $sub ) // array(1) { ["subject"]=> string(20) "DAO Plan de niveaux " } =>> $sub["subject"]
		{
			...
			if (stripos($sub["subject"], "niveaux") !== false || stripos($sub["subject"], "interieur") !== false) {
				switch (true) {
					case ($item->amount >= 200 && $item->amount <= 500):
						$item->value = '1';
						break;
			...
			elseif (stripos($sub["subject"], "façades") !== false || stripos($sub["subject"], "coupes") !== false) {
				switch (true) {
					case ($item->amount >= 100 && $item->amount <= 500):
						$item->value = '1';
						break;
					case ($item->amount >= 501 && $item->amount <= 1000):
						$item->value = '0.8';
