   ...
   <table width="100%" cellspacing="0" >
	  ...
	  <td style="border-bottom:1px solid #ccc;padding-bottom:5px;">
        ...
        <tr>
          ...
          <span class="invoicereference"><?=$this->lang->line('application_estimate');?> <?=$estimate->estimate_num;?></span><br/>
			<?php if ($estimate->project_id != 0){  ?>
				<span class="over">
				<?php echo $estimate->project_ref; ?></span><br/>
			<?php } ?>
			<br>
		  ...
		  <span class="over"><?php if(isset($estimate->subject)){?><?php echo($this->lang->line('application_Object:'))?><?=$estimate->subject;?><?php } ?></span> 
          </td>
...
<table id="table" cellspacing="0" style="margin-top:2px;margin-bottom:0px;"> 
  ...
  <?php $i = 0; $sum = 0; $row=false; ?>
	...
	<?php if ($company->tva == 0){ $totalTVA =$total;?> 
		...
		<tr>
			<td class=""></td><td></td><td></td><td></td><td <?php echo $colspan1; ?> class="right"><?=$this->lang->line('application_tax');?> (+<?=$tax?>%)</td><td class="right"><?=number_format($value,$chiffre,',',' ');?></td>
		</tr>
		 
		<?php $totalTVA = $totalTVA + $value;  endforeach;  ?>  
	<?php } ?>
	<?php
	/*if($estimate->company->timbre_fiscal<1){
	$totalTVA = $totalTVA+$estimate->timbre_fiscal;
	$total = $total+$estimate->timbre_fiscal;
	*/
	?>
    <tr>
   <!-- <td class=""></td><td></td><td></td><td></td><td <?php echo $colspan1; ?> width="20%" class="right"><?=$this->lang->line('application_timbre');?></td><td class="right"><?=number_format($estimate->timbre_fiscal,$chiffre,',',' ');?></td>-->
	</tr> 
		<?php //} ?>
		<?php if($company->guarantee == 1) { 
				if ($company->tva == 1) {?>
					<tr><td class=""></td><td></td><td></td><td <?php echo $colspan1; ?> width="20%" class="right"><?=$this->lang->line('application_guarantee');?>(-10%)</td><td class="right"><?=number_format((($total *10)/100),$chiffre,',',' ');?></td></td>
					</tr> 
				<?php } else { ?>
					<tr><td class=""></td><td></td><td><td></td></td><td <?php echo $colspan1; ?> width="20%" class="right"><?=$this->lang->line('application_guarantee');?>(-10%)</td><td class="right"><?=number_format((($totalTVA *10)/100),$chiffre,',',' ');?></td>
					</tr> 
				<?php } ?>
		<?php }  ?>
...
<table width="100%" height="auto" style="clear:both">
	<tr>
		<?php  if ($estimate->discount != 0): ?><td class="side Right"><span class="over"><?=$this->lang->line('application_discount'); echo('  ('.$estimate->discount.' %)');?></span><br/><span class="under">- <?=display_money($discount, $estimate->currency,$chiffre);?></span></td><?php endif ?>
		 <?php if($company->tva == 0){ ?>
			<td class="side right"><span class="over"><?=$this->lang->line('application_sub_total_HT');?></span><br/><span class="under"><?=number_format($presum,$chiffre,',',' ')." ".$estimate->currency;?></span></td>
		 <?php } ?>
		  ...
		  <td class="side right"><span class="over">
		    ...
		    <?php if($company->guarantee == 1){ ?>
				<?php if ($company->tva == 0 ) { ?>
				</span><br/><span class="under"><?=number_format($total-($totalTVA *10)/100 ,$chiffre,',',' ')." ".$estimate->currency;?></span></td>
				<?php $pr= array('nb' =>$totalTVA-($totalTVA *10)/100 , 'unit' => $estimate->currency); ?>
				<?php } else { ?>
				</span><br/><span class="under"><?=number_format($total-($total *10)/100 ,$chiffre,',',' ')." ".$estimate->currency;?></span>
				<?php $pr= array('nb' =>$total-($total *10)/100 , 'unit' => $estimate->currency); ?></td>
				<?php } ?>
			<?php } else { ?>
			<?php if ($company->tva == 0 ) { ?>
				</span><br/><span class="under"><?=number_format($totalTVA,$chiffre,',',' ')." ".$estimate->currency;?></span>
				<?php $pr= array('nb' =>$totalTVA, 'unit' => $estimate->currency); ?></td>
				<?php } else { ?>
				</span><br/><span class="under"><?=number_format($total,$chiffre,',',' ')." ".$estimate->currency;?></span>
				<?php $pr= array('nb' =>$total, 'unit' => $estimate->currency); ?>
			</td>
    ...
    <div><?php echo " ".$estimate->notes."<br>";?></div><br>
