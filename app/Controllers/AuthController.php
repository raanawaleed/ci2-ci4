...
{
    protected $session, $setingModel, $userModel, $ModuleModel, $db;
    protected $view_data = [];
    ...
    {
        if ($this->session->get('reset_password')) {
            $this->view_data['reset_password'] = true;
            $this->view_data['send_email'] = $this->session->get('send_email');
        }
        $this->session->set('reset_password', false);
        $this->session->set('send_email', false);
    }
    ...
    {
        $this->session->set('current_company', (int)$this->db->table('v_companies')->get()->getRow()->id);
    ...
    {
        if ($this->session->has('current_company')) {
            $this->session->remove('current_company');
        }
