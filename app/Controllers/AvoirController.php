...
use App\Models\RefTypeOccurences;
...
class AvoirController extends BaseController
{
	...
	public function initController(
		...
		$this->referentiels = new RefTypeOccurences();
		$this->refType = new RefType();
		$this->avoirModel = new Avoir();
		$this->avoirHasItem = new AvoirHasItemModel();
		$this->companyModel = new Company();
		$this->client = new Client();  // Assuming you have a ClientModel
		$this->avoirHasItemModel = new AvoirHasItemModel();
		$this->compteBancaireModel = new CompteBancaire();
		$this->avoirHasPaymentModel = new AvoirHasPaymentModel();

		if (!$this->client && (!isset($this->user) || $this->user->admin != 1)) {
	...
	public function create()
	{
		if ($this->request->getPost()) {
			...
			if ($this->request->getPost('company') == 1) {
				...
				$this->request->setPost('company_id', $this->companyModel->getInsertID());
			...
			if (explode('-', $lastAvoir->creation_date)[0] != $year) {
				$this->request->setPost('reference', '01');
			}
			...
			$this->request->setPost('avoir_num', $avoir_num);
	...
	public function view($id = false)
	{
		...
		$this->view_data['project'] = $this->projectModel->where('reference', $avoir->project_id)->first();
	...
	public function preview($id = null, $attachment = false)
	{
		...
		$client = $this->clientModel->find($company->client_id);
		...
		$html = $this->parser->setData($parseData)->parseString($html);
