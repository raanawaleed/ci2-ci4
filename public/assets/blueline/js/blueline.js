...
//Ajax background load
  ...
  var url = $(this).attr('href');
    ...
    }).done(function() { $('.message-list ul li a').first().click(); NProgress.done(); });
...
//Ajax for adding rows
  $(document).on('click','.add-row-ajax',function(){
	...
	$("#addedfield .start_time").html($(".input-fields .start_time").val());
    $(".input-fields .start_time").val("00");
...
$(document).on("click", '.section-reload #send', function (e) {
    ...
    e.preventDefault();
    NProgress.start();
    $('#speakers-tab').load(document.URL +  ' #speakers-tab');
      NProgress.done();
});
...
//Clickable rows
	$(document).on("click", 'table#projects td, table#clients td, table#invoices td, table#cprojects td, table#cinvoices td, table#estimates td, table#cestimates td, table#quotations td, table#messages td, table#cmessages td, table#subscriptions td, table#csubscriptions td, table#tickets td, table#ctickets td', function (e) {
     ...
     $(".chosen-select").chosen({scroll_to_highlighted: false, disable_search_threshold: 4, search_contains:true, width: "100%", display_disabled_options:false, case_sensitive_search:true, no_results_text: "Pas de résultat trouvé"});
       ...
       function uploaderButtons(preClass)
       {
        ...
        function itemSelector()
        {
            $('.additem').click(function(e)
            {
              ...
              $('#item-editor input#name').attr('required', true);
              $('#item-editor input#value').attr('required', true);
              $( "#item-editor input#name" ).addClass( "required" );
              $( "#item-editor input#value" ).addClass( "required" );
              $('#item-editor').delay(300).slideDown('fast');
			  $( "div.Prixunitaire" ).hide();
              $('form').validator();
            });
			...
			$('.addprojectClient').click(function(e)
            {
              $('#item-selector').slideUp('fast');
              $('#item-editor').delay(300).slideDown('fast');
              $('#item-editor input').attr('required', true);
            ...
            {
			  $('#item-editor').slideUp('fast');
			  $('#item-selector').delay(300).slideDown('fast');
			  $('#item-selector input').attr('required', true);
            });
...
function sorting_list(baseurl){
    $( ".sortable-list" ).sortable({
      ...
      receive: function( event, ui )
      {
                ...
                taskId = event.toElement.parentElement.id;
...
function taskviewer()
{
		...
		//click tache, detailles par deafaut
		 $(document).on("click", '#task_menu_link', function (e) {
			$(".todo-details").show();
		 });
		//on task click
...
$('input[type=text][readonly]').css('color','red');
// Test sur la quantité dans un article //
